...
jQuery(document).ready(function ($) {
    ...
    if (wooscpVars.open_button != '') {
        $('body').on('click touch', wooscpVars.open_button, function () {
            if ((wooscpVars.open_button_action == 'open_page') && (wooscpVars.page_url != '')) {
                // open compare page
...
function wooscpChangeCount(count) {
    ...
    jQuery('.wooscp-menu-item').each(function () {
        if (jQuery(this).hasClass('menu-item-type-wooscp')) {
            ...
            jQuery(this).addClass('menu-item-type-wooscp').find('a').wrapInner('<span class="wooscp-menu-item-inner" data-count="' + count +
                '"></span>');
        }
    });
